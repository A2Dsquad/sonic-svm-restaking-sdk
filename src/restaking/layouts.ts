...
import { option, u64, vec } from './codecs';
...
export class FutureEpochLayout<T> extends Layout<T | null> {
  layout: Layout<T>;
...
export const StakePoolLayout = struct<StakePool>([
  ...
  u64('poolTokenSupply'),
  u64('lastUpdateEpoch'),
  struct<Lockup>([u64('unixTimestamp'), u64('epoch'), publicKey('custodian')], 'lockup'),
  struct<Fee>(feeFields, 'epochFee'),
  futureEpoch<any>(struct(feeFields), 'nextEpochFee'),
  option(publicKey(), 'preferredDepositValidatorVoteAddress'),
  option(publicKey(), 'preferredWithdrawValidatorVoteAddress'),
  struct<Fee>(feeFields, 'stakeDepositFee'),
  struct<Fee>(feeFields, 'stakeWithdrawalFee'),
  futureEpoch(struct<Fee>(feeFields), 'nextStakeWithdrawalFee'),
  u8('stakeReferralFee'),
  option(publicKey(), 'solDepositAuthority'),
  struct<Fee>(feeFields, 'solDepositFee'),
  u8('solReferralFee'),
  option(publicKey(), 'solWithdrawAuthority'),
  struct<Fee>(feeFields, 'solWithdrawalFee'),
  futureEpoch(struct<Fee>(feeFields), 'nextSolWithdrawalFee'),
